...
# Параметры страницы
st.set_page_config(
    page_title="Ахмад Академиясы - ҰБТ Порталы", page_icon="🎓", layout="centered"
)
...
DEFAULT_QUESTIONS = [
    ...
    },
    {
        ...
        "type": "multiple",
        "question": (
            "Төмендегілердің қайсысы скаляр шамалар болып табылады? (Бірнеше"
            " жауап таңдаңыз)"
        ),
        "image": "",
...
# СЕССИЯ ЖӘНЕ ПАЙДАЛАНУШЫЛАРДЫ БАСҚАРУ (Attempts, lockout_until, expires_at қосылды)
...
if "users" not in st.session_state:
  st.session_state.users = {
      ...
      "teacher": {
          ...
          "attempts": 0,
          "lockout_until": None,
          "expires_at": datetime.now() + timedelta(days=30),  # Мұғалімге 1 ай лимит
      },
      "admin": {
          "password": "admin123",
          "name": "Мектеп Директоры",
          "role": "director",
          "active_session_id": "",
          "is_blocked": False,
          "attempts": 0,
...
# КІРУ СРАНИЦАСЫ (LOGIN - 10 рет қате болса 30 минутқа блоктау логикасы қосылды)
def login_page():
  ...
  if st.button("Кіру", type="primary", use_container_width=True):
    if username in st.session_state.users:
      ...
      # 1. 30 минуттық бұғаттауды тексеру
      if user.get("lockout_until") and now < user.get("lockout_until"):
        rem_mins = int((user["lockout_until"] - now).total_seconds() / 60 + 1)
        st.error(
            f"🚫 Парольді 10 рет қате енгізгендіктен аккаунт 30 минутқа"
            f" бұғатталған! Қалған уақыт: ~{rem_mins} минут."
        )
        return

      # Егер бұғаттау уақыты өтіп кетсе, attempts-ті тазалаймыз
      if user.get("lockout_until") and now >= user.get("lockout_until"):
        user["lockout_until"] = None
        user["attempts"] = 0

      # 2. Әкімшілік бұғаттауды тексеру
      if user.get("is_blocked", False):
        st.error(
            "🚫 Бұл аккаунт бұғатталған! Директорға немесе әкімшіге"
            " хабарласыңыз."
        )
      # 3. Мұғалімнің 1 айлық лимит уақытын тексеру
      elif (
          user["role"] == "teacher"
          and user.get("expires_at")
          and now > user["expires_at"]
      ):
        st.error(
            "❌ Сіздің мұғалімдік лимит мерзіміңіз (1 ай) аяқталды! Директордан"
            " қайта рұқсат алуыңыз қажет."
        )
      elif user["password"] == password:
        # Пароль дұрыс болса қателерді нөлдейміз
        user["attempts"] = 0
        user["lockout_until"] = None
        ...
        st.session_state.teacher_subject = user.get(
            "subject", "Математикалық сауаттылық"
        )
        st.session_state.tab_warnings = 0
        st.rerun()
      else:
        # Қате пароль логикасы (10 рет қате -> 30 мин бан)
        user["attempts"] = user.get("attempts", 0) + 1
        rem_attempts = 10 - user["attempts"]

        if user["attempts"] >= 10:
          user["lockout_until"] = datetime.now() + timedelta(minutes=30)
          st.error(
              "🚨 Пароль 10 рет қате енгізілді! Аккаунт қауіпсіздік мақсатында"
              " 30 минутқа бұғатталды."
          )
        else:
          st.error(
              f"❌ Пароль қате! Қалған қате әрекет саны: {rem_attempts}"
          )
    else:
...
def student_dashboard():
  ...
  if not st.session_state.test_started and not st.session_state.test_finished:
    ...
    filtered_q = [
        ...
        if q.get("direction") == selected_subject
    ]
    ...
    if warnings == 1 or warnings == 2:
      st.warning(
          f"⚠️ **Ескерту ({warnings}/4):** Тест кезінде басқа вкладкаға өтуге"
          " болмайды!"
      )
    elif warnings == 3:
      st.warning(
          f"⚠️ **Ескерту (3/4):** Қайтадан басқа вкладкаға өтпеңіз! Соңғы"
          " мүмкіндік!"
      )
    elif warnings == 4:
      st.error(
          "🚨 **НАЗАР АУДАРЫҢЫЗ!** Бұл 4-ші ескерту. Тағы 1 рет ауыстырсаңыз,"
          " аккаунт БҰҒАТТАЛАДЫ!"
      )
    elif warnings >= 5:
      ...
      st.session_state.test_started = False
      st.error(
          "🚫 Ереже бұзылды! Вкладканы 5 рет ауыстырғаныңыз үшін аккаунтыңыз"
          " бұғатталды!"
      )
      st.rerun()
    ...
    filtered_questions = [
        q for q in st.session_state.questions if q.get("direction") == subject
    ]
...
def teacher_dashboard():
  ...
  q_type = st.radio(
      "Сұрақтың түрі:",
      ["Бір дұрыс жауапты", "Көп дұрыс жауапты (бірнеше)"],
      key="t_add_type",
  )
  q_text = st.text_input("Сұрақтың мәтіні:", key="t_add_text")
  img_url = st.text_input(
      "Суреттің URL сілтемесі (міндетті емес):",
  ...
  options_list = [
      opt for opt in [opt1, opt2, opt3, opt4] if opt.strip() != ""
  ]
  ...
  if len(options_list) == 4:
    ...
    if q_type == "Бір дұрыс жауапты":
      selected_single = st.selectbox(
          "Дұрыс жауапты таңдаңыз:", options_list, key="t_ans_single"
      )
      correct_ans = [selected_single] if selected_single else []
    else:
      correct_ans = st.multiselect(
          "Дұрыс жауаптарды белгілеңіз:", options_list, key="t_ans_multi"
      )
...
# ДИРЕКТОР КАБИНЕТІ (Тек Директор бере алатын 1 айлық лимит және басқару құралдары)
def director_dashboard():
  st.title(f"👨‍💼 Директор кабинеті")

  tab1, tab2, tab3 = st.tabs([
      "📊 Оқушылар нәтижесі",
      "🔑 Аккаунттар & Мұғалім лимиті (1 ай)",
      "🔍 Сұрақтарды басқару & Өшіру",
  ])
  ...
  with tab2:
    st.subheader("👥 Пайдаланушылар тізімі және Мұғалім лимиттері")
    user_list = []
    for uname, udata in st.session_state.users.items():
      role_str = "Оқушы"
      if udata["role"] == "director":
        role_str = "Директор"
      elif udata["role"] == "teacher":
        exp_str = (
            udata["expires_at"].strftime("%Y-%m-%d")
            if udata.get("expires_at")
            else "Лимит жоқ"
        )
        role_str = f"Мұғалім ({udata.get('subject')}) [Лимит: {exp_str}]"

      status_str = (
          "🚫 Бұғатталған" if udata.get("is_blocked", False) else "✅ Белсенді"
      )
    ...
    st.divider()
    st.subheader("⚙️ Мұғалімдерге 1 айлық лимит беру / ұзарту және өңдеу")

    all_logins = list(st.session_state.users.keys())
    selected_user_to_edit = st.selectbox(
        "Пайдаланушыны таңдаңыз:", all_logins, key="dir_sel_user"
    )

    if selected_user_to_edit:
      u_info = st.session_state.users[selected_user_to_edit]
      edit_name = st.text_input(
          "Аты-жөні:", value=u_info["name"], key="edit_u_name"
      )
      edit_pass = st.text_input(
          "Пароль:", value=u_info["password"], key="edit_u_pass"
      )
      is_blocked_status = st.checkbox(
          "🚫 Аккаунт бұғатталған (Бұғаттан шығару үшін белгіні алыңыз)",
          value=u_info.get("is_blocked", False),
          key="edit_u_block",
      )

      # Егер таңдалған қолданушы мұғалім болса, Директор 1 ай лимит бере алады немесе тоқтата алады
      if u_info["role"] == "teacher":
        current_exp = (
            u_info["expires_at"].strftime("%Y-%m-%d %H:%M")
            if u_info.get("expires_at")
            else "Жоқ"
        )
        st.write(f"**Ағымдағы мұғалім лимитінің бітетін күні:** {current_exp}")

        col_l1, col_l2 = st.columns(2)
        with col_l1:
          if st.button("⏳ 1 айлық лимит беру / Ұзарту"):
            st.session_state.users[selected_user_to_edit]["expires_at"] = (
                datetime.now() + timedelta(days=30)
            )
            st.success(
                f"'{selected_user_to_edit}' мұғаліміне 1 айлық лимит сәтті"
                " берілді!"
            )
            st.rerun()
        with col_l2:
          if st.button("❌ Лимитті алып тастау (Мұғалімді тоқтату)"):
            st.session_state.users[selected_user_to_edit]["expires_at"] = (
                datetime.now() - timedelta(days=1)
            )  # Мерзімін өткізіп жіберу арқылы тоқтату
            st.success(
                f"'{selected_user_to_edit}' мұғалімінің лимиті тоқтатылды!"
            )
            st.rerun()
      ...
      with col_u_save:
        if st.button("💾 Мәліметтерді сақтау", type="primary"):
          ...
          st.session_state.users[selected_user_to_edit]["is_blocked"] = (
              is_blocked_status
          )
    ...
    st.divider()
    st.subheader("### ➕ Жаңа пайдаланушы тіркеу (Директор тарапынан)")
    ...
    new_role = st.selectbox(
        "Ролі:",
        ["student", "teacher"],
        format_func=lambda x: "Оқушы" if x == "student" else "Мұғалім",
        key="new_u_role",
    )

    teacher_subj = None
    if new_role == "teacher":
      teacher_subj = st.selectbox(
          "Мұғалімге берілетін пән:", ALL_SUBJECTS, key="new_u_subj"
      )

    if st.button("Тіркеу", key="new_u_btn"):
  ...
  with tab3:
    ...
    with col_s1:
      search_query = (
          st.text_input(
              "🔍 Іздеу (мәтін немесе автор аты):",
              placeholder="Іздеу сөзі...",
              key="dir_search_q",
          )
          .strip()
          .lower()
    ...
    with col_s2:
      filter_subj = st.selectbox(
          "Пән бойынша сүзгі:", ["Барлығы"] + ALL_SUBJECTS, key="dir_filter_subj"
      )
    ...
    if not matching_questions:
      ...
      q_options = [
          f"{idx+1}. [{q.get('direction', 'Пәнсіз')}] {q['question']}"
          for idx, (real_idx, q) in enumerate(matching_questions)
      ]
      selected_match_idx = st.selectbox(
          "Басқару үшін сұрақты таңдаңыз:",
          range(len(q_options)),
          format_func=lambda x: q_options[x],
          key="dir_search_select",
      )
      ...
      st.divider()
      st.info(
          f"✍️ **Бұл сұрақтың авторы:** {q_data.get('author', 'Белгісіз')}"
      )

      edit_subj_index = (
          ALL_SUBJECTS.index(q_data["direction"])
          if q_data.get("direction") in ALL_SUBJECTS
          else 0
      )
      edit_subj = st.selectbox(
          "Пәні:", ALL_SUBJECTS, index=edit_subj_index, key="dir_edit_subj"
      )
      edit_text = st.text_input(
          "Сұрақ мәтіні:", value=q_data["question"], key="dir_edit_text"
      )
      ...
      with col2:
        ...
        e_opt4 = st.text_input(
            "D нұсқасы:",
            value=opts[3] if len(opts) > 3 else "",
            key="dir_e_opt4",
        )
...
if st.session_state.logged_in:
  ...
  active_session_in_db = (
      st.session_state.users.get(current_username, {})
  ...
  if current_session_id != active_session_in_db:
    st.session_state.logged_in = False
    st.warning(
        "⚠️ Сіздің аккаунтыңызға басқа құрылғыдан кірді! Жүйеден"
        " шығарылдыңыз."
    )
    st.rerun()
